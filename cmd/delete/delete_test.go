...
func TestDeleteCommand(t *testing.T) {
	...
	assert.Equal(t, len(deleteCommand.Commands()), 5)
	cmdNouns := deleteCommand.Commands()
	// Sort by Use value.
	sort.Slice(cmdNouns, func(i, j int) bool {
		return cmdNouns[i].Use < cmdNouns[j].Use
	})
	useArray := []string{"cluster-resource-attribute", "execution", "execution-cluster-label", "execution-queue-attribute", "task-resource-attribute"}
