...
func TestGetWorkflowExecutionConfig(t *testing.T) {
	...
	t.Run("successful get project domain attribute", func(t *testing.T) {
		...
		tearDownAndVerify(t, s.Reader, `{"project":"dummyProject","domain":"dummyDomain","max_parallelism":5}`)
	})
	...
	t.Run("successful get workflow attribute", func(t *testing.T) {
		...
		tearDownAndVerify(t, s.Reader, `{"project":"dummyProject","domain":"dummyDomain","workflow":"workflow","max_parallelism":5}`)
	})
