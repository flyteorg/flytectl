...
func TestCreateGetCommand(t *testing.T) {
	...
	assert.Equal(t, len(getCommand.Commands()), 9)
	...
	useArray := []string{"cluster-resource-attribute", "execution", "execution-cluster-label",
		"execution-queue-attribute", "launchplan", "project", "task", "task-resource-attribute", "workflow"}
	aliases := [][]string{{"cluster-resource-attributes"}, {"executions"}, {"execution-cluster-labels"},
		{"execution-queue-attributes"}, {"launchplans"}, {"projects"}, {"tasks"}, {"task-resource-attributes"}, {"workflows"}}
	shortArray := []string{clusterResourceAttributesShort, executionShort, executionClusterLabelShort, executionQueueAttributesShort, launchPlanShort, projectShort,
		taskShort, taskResourceAttributesShort, workflowShort}
	longArray := []string{clusterResourceAttributesLong, executionLong, executionClusterLabelLong, executionQueueAttributesLong, launchPlanLong, projectLong, taskLong,
