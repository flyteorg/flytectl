...
func newRootCmd() *cobra.Command {
	...
	rootCmd.PersistentFlags().Int32Var(&(config.GetConfig().Limit), "limit", config.DefaultLimit, "Specifies the limit on results")
	rootCmd.PersistentFlags().BoolVar(&(config.GetConfig().Asc), "asc", false, "Specifies the sorting order")
